...
function normalizeForm(value?: AssignmentFormValue | null) {
  return {
...
function isSameForm(a?: AssignmentFormValue | null, b?: AssignmentFormValue | null) {
...
export default function KitchenSettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"layout" | "assignments">("layout");
  ...
  const [initialAssignmentForm, setInitialAssignmentForm] = useState<
    Record<string, AssignmentFormValue>
  >({});
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [{ data: tablesData }, { data: studentsData }, { data: assignmentsData }] =
        await Promise.all([
  ...
  const handleDeleteTable = (table: KitchenTable) => {
    Alert.alert(
      "Delete Table",
      `Remove ${table.table_name || `Table ${table.table_number}`}?`,
      [
  ...
  const saveAssignmentForStudent = async (studentUid: string) => {
    ...
    try {
      ...
      setRowSavingMap((prev) => ({ ...prev, [studentUid]: false }));
    }
  };

  const studentClasses = useMemo(() => {
    return Array.from(new Set(students.map((s) => s.class_id || "Unassigned"))).sort();
  }, [students]);
  ...
  const visibleAssignmentStudents = useMemo(() => {
    return studentsByClass[activeAssignmentClass] || [];
  }, [studentsByClass, activeAssignmentClass]);

  const visibleDirtyCount = useMemo(() => {
    return visibleAssignmentStudents.filter((student) => {
      return !isSameForm(
        assignmentForm[student.student_uid],
        initialAssignmentForm[student.student_uid]
      );
    }).length;
  ...
  const handleSaveAllVisible = async () => {
    ...
    if (visibleDirtyCount === 0) {
      Alert.alert("Nothing to Save", "There are no unsaved changes in this class.");
      return;
    }
    ...
    try {
      ...
      for (const student of visibleAssignmentStudents) {
        ...
        if (!currentForm.tableId || !currentForm.seatNumber) {
          ...
          const payload = {
            kitchen_table_id: currentForm.tableId,
            seat_number: Number(currentForm.seatNumber),
  ...
  const handleClearVisible = () => {
    ...
    Alert.alert(
      "Clear Visible Class",
      `Clear assignment form values for all students in ${activeAssignmentClass}?`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Clear",
  ...
  const handleTabChange = (tab: "layout" | "assignments") => {
        ...
        >
          <View style={styles.heroCard}>
            ...
            <Text style={styles.heroSubtitle}>
              Manage table layout, seat counts, and student assignments.
            </Text>
          </View>
          ...
          {switchingTab ? (
            <View style={styles.transitionCard}>
              ...
              <Text style={styles.transitionText}>Opening {activeTab}...</Text>
            </View>
          ) : activeTab === "layout" ? (
            <View style={styles.stack}>
              <View style={styles.card}>
                <SectionTitle
                  ...
                  subtitle="Default tables start with 8 seats horizontally."
                />
                ...
                <View key={table.id} style={styles.card}>
                  <View style={styles.cardHeaderRow}>
                    <View style={styles.cardHeaderTextWrap}>
                      ...
                      <Text style={styles.cardDesc}>
                        Configure row, position, orientation, and seats
                      </Text>
                    </View>
                    ...
                    <TouchableOpacity
                      ...
                      style={[
                        styles.statusBadge,
                        table.is_active ? styles.badgeActive : styles.badgeInactive,
                      ]}
                      activeOpacity={0.84}
                    >
                      <Text
                        style={
                          table.is_active ? styles.badgeTextActive : styles.badgeTextInactive
                        }
                      >
                  ...
                  <View style={styles.formGrid}>
                    <View style={styles.inputWrap}>
                      ...
                      <TextInput
                        ...
                        onChangeText={(val) =>
                          setTables((prev) =>
                            prev.map((t) =>
                              t.id === table.id ? { ...t, table_name: val } : t
                            )
                          )
                        }
                    ...
                    <View style={styles.inputWrap}>
                      ...
                      <TextInput
                        style={styles.input}
                        value={String(table.row_number)}
                        keyboardType="number-pad"
                        onChangeText={(val) =>
                          setTables((prev) =>
                            prev.map((t) =>
                              t.id === table.id
                                ? { ...t, row_number: Number(val) || 1 }
                                : t
                            )
                          )
                        }
                      ...
                      <CustomPicker
                        ...
                        onSelect={(val) =>
                          setTables((prev) =>
                            prev.map((t) =>
                              t.id === table.id
                                ? { ...t, row_position: val as RowPosition }
                                : t
                            )
                          )
                        }
                      ...
                      <CustomPicker
                        ...
                        onSelect={(val) =>
                          setTables((prev) =>
                            prev.map((t) =>
                              t.id === table.id
                                ? { ...t, orientation: val as Orientation }
                                : t
                            )
                          )
                        }
                      ...
                      <CustomPicker
                        ...
                        onSelect={(val) =>
                          setTables((prev) =>
                            prev.map((t) =>
                              t.id === table.id
                                ? { ...t, active_seat_count: Number(val) }
                                : t
                            )
                          )
                        }
            ...
            <View style={styles.stack}>
              <View style={styles.card}>
                <SectionTitle
                  title="Assignment by Class"
                  subtitle="Open one class at a time for faster editing."
                />
                ...
                >
                  {studentClasses.map((classId) => (
                    <TouchableOpacity
                      key={classId}
                      onPress={() => setActiveAssignmentClass(classId)}
                      style={[
                        styles.classChip,
                        activeAssignmentClass === classId && styles.classChipActive,
                      ]}
                      activeOpacity={0.84}
                    >
                      <Layers3
                      ...
                      <Text
                        style={[
                          ...
                          activeAssignmentClass === classId && styles.classChipTextActive,
                        ]}
                      >
                        {classId}
                      </Text>
                ...
                <View style={styles.assignmentSummaryRow}>
                  <View style={styles.assignmentSummaryBox}>
                    <Text style={styles.assignmentSummaryValue}>
                      {visibleAssignmentStudents.length}
                    </Text>
                    <Text style={styles.assignmentSummaryLabel}>Visible Students</Text>
                  </View>

                  <View style={styles.assignmentSummaryBox}>
                    <Text style={styles.assignmentSummaryValue}>{visibleDirtyCount}</Text>
                    <Text style={styles.assignmentSummaryLabel}>Unsaved Changes</Text>
                  </View>
                </View>

                <View style={styles.bulkAssignmentRow}>
                  <TouchableOpacity
                    onPress={handleSaveAllVisible}
                    style={[
                      styles.btnPrimary,
                      (bulkSaving || visibleDirtyCount === 0) && styles.dimmedButton,
                    ]}
                    activeOpacity={0.84}
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      styles.btnOutlineWide,
                      clearingVisible && styles.dimmedButton,
                    ]}
                    activeOpacity={0.84}
              ...
              {visibleAssignmentStudents.map((student) => {
                ...
                const initialForm = initialAssignmentForm[student.student_uid] || {
                  tableId: "",
                  seatNumber: "",
                };
                ...
                const seatOptions = assignedTable
                  ? Array.from({ length: assignedTable.active_seat_count }, (_, i) => ({
                      label: `Seat ${i + 1}`,
                      value: String(i + 1),
                    }))
                  : [];
                  ...
                  <View key={student.student_uid} style={styles.card}>
                    <View style={styles.assignmentCardHeader}>
                      <View style={styles.cardHeaderTextWrap}>
                        ...
                        <Text style={styles.cardDesc}>
                          {student.class_id} • CIC: {student.cic || "—"}
                        </Text>
                      </View>
                      ...
                      <View
                        style={[
                          styles.changeBadge,
                          isDirty ? styles.changeBadgeActive : styles.changeBadgeIdle,
                        ]}
                      >
                        <Text
                          style={[
                            ...
                            isDirty
                              ? styles.changeBadgeTextActive
                              : styles.changeBadgeTextIdle,
                          ]}
                        >
                      ...
                      <View style={styles.inputWrap}>
                        ...
                        <CustomPicker
                          ...
                          options={tables
                            ...
                            .map((t) => ({
                              label: t.table_name || `Table ${t.table_number}`,
                              value: t.id,
                            }))}
                          onSelect={(val) =>
                            setAssignmentForm((prev) => ({
                              ...prev,
                              [student.student_uid]: { tableId: val, seatNumber: "" },
                            }))
                          }
                        />
                      </View>
                    ...
                    <View style={styles.cardActions}>
                      <TouchableOpacity
                        onPress={() => saveAssignmentForStudent(student.student_uid)}
                        style={[
                          styles.btnPrimary,
                          (!isDirty || isSaving) && styles.dimmedButton,
                        ]}
                        activeOpacity={0.84}
                      ...
                      <TouchableOpacity
                        onPress={() =>
                          setAssignmentForm((prev) => ({
                            ...prev,
                            [student.student_uid]: { tableId: "", seatNumber: "" },
                          }))
                        }
