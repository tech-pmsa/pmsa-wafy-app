...
import { View, Text, ScrollView, TouchableOpacity, ActivityIndicator, Image, Modal, TextInput, Alert as NativeAlert } from 'react-native';
import * as ImagePicker from 'expo-image-picker';
import * as FileSystem from 'expo-file-system/legacy';
import { decode } from 'base64-arraybuffer';
import { supabase } from '@/lib/supabaseClient';
import { useUserData } from '@/hooks/useUserData';
import { User, Briefcase, Building, Shield, UserCheck, Phone, PhoneCall, Home, BookMarked, Camera, Pencil, Save, X } from 'lucide-react-native';
...
import { EmailChangeModal } from './profile/EmailChangeModal'; // Ensure this doesn't have a <Modal> inside a <Modal>
...
export default function ProfileSection() {
  ...
  // Modal Control States - Kept at the root level!
  ...
  const fetchExtraData = useCallback(async () => {
    ...
    const [academicRes, familyRes] = await Promise.all([
      supabase.from('academic_entries').select('*, subject_marks(*)').eq('student_uid', user.id).order('created_at'),
  ...
  useEffect(() => {
    if (details) setPersonalForm(details);
    fetchExtraData();
  }, [details, fetchExtraData]);

  // --- Native Image Picker & Cropper ---
  const handleAvatarUpdate = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        const { error: updateError } = await supabase.from(table).update({ img_url: newUrl }).eq('uid', user?.id);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      const updatedData = isStudent ? { name, phone, guardian, g_phone, address } : { name, designation, batch };
  ...
  if (loading || !details) return <ActivityIndicator size="large" color="#09090b" className="my-10" />;

  return (
    <View className="pb-10">
      {/* Top Profile Card */}
      <View className="bg-white rounded-3xl p-6 shadow-sm border border-zinc-200 items-center mb-6">
        <TouchableOpacity onPress={handleAvatarUpdate} disabled={isSaving} className="relative mb-4">
          <View className="h-32 w-32 rounded-full border-4 border-zinc-50 items-center justify-center overflow-hidden bg-zinc-100 shadow-sm">
        ...
        <View className="bg-zinc-100 px-3 py-1 rounded-full mt-2">
          <Text className="text-xs font-bold text-zinc-600 uppercase tracking-wider">{details.role}</Text>
        </View>
      ...
      <View className="flex-row bg-zinc-200 p-1 rounded-xl mb-6">
        <TouchableOpacity onPress={() => setActiveTab('personal')} className={`flex-1 py-3 rounded-lg items-center ${activeTab === 'personal' ? 'bg-white shadow-sm' : ''}`}>
          <Text className={`font-semibold ${activeTab === 'personal' ? 'text-zinc-900' : 'text-zinc-500'}`}>Personal</Text>
        </TouchableOpacity>
        {isStudent && (
          <>
            <TouchableOpacity onPress={() => setActiveTab('academics')} className={`flex-1 py-3 rounded-lg items-center ${activeTab === 'academics' ? 'bg-white shadow-sm' : ''}`}>
              <Text className={`font-semibold ${activeTab === 'academics' ? 'text-zinc-900' : 'text-zinc-500'}`}>Academics</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setActiveTab('family')} className={`flex-1 py-3 rounded-lg items-center ${activeTab === 'family' ? 'bg-white shadow-sm' : ''}`}>
              <Text className={`font-semibold ${activeTab === 'family' ? 'text-zinc-900' : 'text-zinc-500'}`}>Family</Text>
            </TouchableOpacity>
          </>
        )}
      </View>
      ...
      <View className="bg-white rounded-3xl p-5 shadow-sm border border-zinc-200">
        <View className="flex-row justify-between items-center mb-6">
          <Text className="text-xl font-bold text-zinc-900">
            {activeTab === 'personal' ? 'Personal Details' : activeTab === 'academics' ? 'Academic Records' : 'Family Data'}
          </Text>
          {activeTab === 'personal' && (
            <TouchableOpacity onPress={() => setEditOpen(true)} className="bg-zinc-100 p-2.5 rounded-full">
        ...
        {activeTab === 'personal' && (
          <View>
            ...
            {/* The Email Change Modal button sits here, but its Modal root is rendered below */}
            <EmailChangeModal />
          </View>
        )}
        ...
        {activeTab === 'academics' && (
          <AcademicsTab
            ...
            onAdd={() => { setSelectedAcademicEntry(null); setIsMarkModalOpen(true); }}
            onEdit={(entry: any) => { setSelectedAcademicEntry(entry); setIsMarkModalOpen(true); }}
            onDelete={handleDeleteAcademicEntry}
          />
        )}

        {activeTab === 'family' && (
          <FamilyDataTab data={familyData} />
        )}
      </View>

      {/* ========================================================================= */}
      {/* ALL MODALS PLACED AT THE ROOT LEVEL OF THIS COMPONENT TO PREVENT NESTING BUGS */}
      {/* ========================================================================= */}
      ...
      <Modal visible={editOpen} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setEditOpen(false)}>
        <View className="flex-1 bg-zinc-100 pt-6 px-6">
          <View className="flex-row justify-between items-center mb-6">
            ...
            <TouchableOpacity onPress={() => setEditOpen(false)}><X size={24} color="#09090b" /></TouchableOpacity>
          </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <View className="space-y-4 pb-10">
              <View>
                ...
                <TextInput className="bg-white border border-zinc-200 rounded-xl p-4 text-base" value={personalForm.name} onChangeText={(t) => setPersonalForm({...personalForm, name: t})} />
              </View>
              {isStudent ? (
                <>
                  <View><Text className="text-sm font-medium text-zinc-700 mb-1">Phone Number</Text><TextInput className="bg-white border border-zinc-200 rounded-xl p-4 text-base" value={personalForm.phone} keyboardType="phone-pad" onChangeText={(t) => setPersonalForm({...personalForm, phone: t})} /></View>
                ...
                <>
                  <View><Text className="text-sm font-medium text-zinc-700 mb-1">Designation</Text><TextInput className="bg-white border border-zinc-200 rounded-xl p-4 text-base" value={personalForm.designation} onChangeText={(t) => setPersonalForm({...personalForm, designation: t})} /></View>
                </>
              )}

              <TouchableOpacity onPress={handleSaveProfile} disabled={isSaving} className="w-full py-4 rounded-xl flex-row justify-center items-center bg-zinc-900 mt-4">
